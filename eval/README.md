...
- generic-praise ceiling: the share of `✔` lines that are stock or reused must not exceed `GRAMMAR_EVAL_MAX_PRAISE_GENERIC_RATE` (default `0.25`). The pre-#25 prompt scored ~100% generic on live sampling - twenty samples drew on six stock verdicts, `Clean and natural.` alone taking half of them - so the ceiling separates the old prompt from the new one with a wide margin. It is not zero because two unrelated messages can honestly earn the same construction praise, and such a collision costs both samples.
