...
- **praise-category-label** - a praise line is a short quoted fragment plus a label, and the label is the half that can teach. A label that only names the category (`"the log" - correct definite article usage`) repeats the bracket tag the error lines already print; one that states the rule (`"the log" - definite because you named it above`) is something the user can apply to the next sentence. The metric is the share of praise lines whose label, with quotes stripped, is nothing but `correct/proper/good/nice/right <category words>`, and it has a permissive ceiling: a three-word imperative may genuinely have no rule worth teaching, and forcing one would invite invention.
- **praise-generic** - not a case class but a property checked over every `✔` line the run produces, in any class: the praise must be about the message it was written for (issue #25). Two mechanical signals count a compliment as generic - it matches one of the stock verdicts listed in `STOCK_PRAISE` (the ones the prompt bans, plus the `Looks good` fallback the hook writes when the model returns nothing usable), or the identical compliment was written for a second, different message. Neither signal proves a compliment is genuinely insightful; together they catch the failure the issue reports, a praise line that carries no information about the message. Gated by a ceiling on the generic rate. Praise is read from the isolated status file rather than `history.jsonl`, even though the hook now logs it in both places: the status file is what the user actually reads, and scoring it exercises the display path including the `Looks good` substitution.
...
- generic-praise ceiling: the share of `✔` lines that are stock or reused must not exceed `GRAMMAR_EVAL_MAX_PRAISE_GENERIC_RATE` (default `0.25`).
- category-label ceiling: the share of praise labels that only name the category must not exceed `GRAMMAR_EVAL_MAX_PRAISE_CATEGORY_RATE` (default `0.5`). A matched A/B over the same 34 messages scored the prompt before the rule-teaching wording at 74% and after it at 26%, so the ceiling separates them with margin on both sides. The pre-#25 prompt scored ~100% generic on live sampling - twenty samples drew on six stock verdicts, `Clean and natural.` alone taking half of them - so the ceiling separates the old prompt from the new one with a wide margin. It is not zero because two unrelated messages can honestly earn the same construction praise, and such a collision costs both samples.
