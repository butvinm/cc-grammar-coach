...
The shipped set is the author's default selection, not a completeness claim: which error types are worth flagging is a per-user choice, and a type absent here may be exactly what another user needs coached. Splitting the config into a full catalog plus a per-user selection is #15. The table therefore distinguishes types the selection covers, types left out of the selection (with a pointer to what the current behavior is, not a universal justification), and types that were never considered until this table was written.
...
| MORPH                                                 | `word-form`                                                                                                                                                                                                      |
| WO, M:VERB:AUX in questions                           | `questions` covers only do-support and subject-auxiliary inversion; other word order was never considered before this table - undefined, see #14                                                                 |
| PRON, ADJ:FORM, PART, NOUN:POSS                       | never considered before this table - undefined, see #14                                                                                                                                                          |
| SPELL, ORTH, PUNCT, CONTR                             | not selected: the checker's silence list names typos, capitalization, and punctuation explicitly (`prompts/checker.txt` lines 5-11)                                                                              |
| NOUN:INFL, VERB:INFL                                  | not selected: malformed non-words ("catched", "informations") fall under the real-words rule (`prompts/checker.txt` line 3) and read as misspellings                                                             |
| ADJ, ADV, NOUN, VERB, CONJ (word-choice replacements) | not selected: word-choice corrections currently surface only in the ✨ rephrase line                                                                                                                             |
