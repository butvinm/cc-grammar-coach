...
- `drills/*.json` - authored drill/lesson data, one file per session, named `drill-<date>-<HHMMSS>.json`, schema = existing authoring schema plus a top-level `kind: "drill" | "learn"` badge (stamped deterministically by the prepare script, not authored by the LLM). Legacy `drills/*.html` files sit alongside, ignored by the dashboard.
...
- [x] `prepare_drill.py --kind drill|learn <data.json>`: validate authored data with the existing `validate()`, stamp `kind`, write `$GRAMMAR_HOME/drills/drill-<date>-<HHMMSS>.json`, print the filename - refuse to write on any validation failure or missing/invalid `--kind`
...
- blocker fix (data loss, flagged independently by the drill and the learn run): `prepare_drill.py` named its output `drill-<date>-<HHMM>.json` with no collision guard, so two sessions prepared in the same minute - drill then learn back to back is a plausible session - silently overwrote each other. The name now carries seconds (`drill-<date>-<HHMMSS>.json`) and the file is created with an exclusive open, so two runs in one minute produce two files and a run that would land on an existing name fails loudly instead of overwriting it
...
- note: every name is now the same length, so the lexical `sorted(reverse=True)` behind `/api/drills` is chronological; the only case it cannot order is two sessions inside one second, which the exclusive open refuses to create at all
...
- [ ] move this plan to `docs/plans/completed/` - the exec harness does this at the very end of the run, so the box stays open while the file is still here
...
- correction (this line previously recorded a follow-up that does not exist): the quiz card does not clip when the viewport is shorter than the tallest question needs - it scrolls. `fit()` clamps `--drill-body-h` to `avail`, and `.duo-shell__body` is `overflow-y: auto`, so the clamped body becomes a scroll container. Measured on the 18-question drill at both window heights, answering wrongly so the tall feedback box is the one on screen: at 794px `scrollHeight` 684 vs `clientHeight` 542, at 493px 684 vs 241, `overflow-y` computed `auto` in both, and after scrolling to the bottom the feedback box including its explanation line is fully inside the body rect. The screenshot decision above stands on its own ground - a 1000px-tall window shows the tallest question without any scrolling - but nothing is lost at 794px either
