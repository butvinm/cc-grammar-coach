...
| `history.jsonl`                              | Append-only mistake log, one JSON object per reviewed message. The drill's data source.                                                                                                                                                                                         |
| `drills/`                                    | Authored quiz sessions as JSON, one per drill or lesson, so a session outlives the conversation that ran it.                                                                                                                                                                    |
| `results.jsonl`                              | One line per finished quiz: the tally and the per-topic breakdown.                                                                                                                                                                                                              |
| `skip-next-prompt`                           | A one-shot token dropped by a quiz before it asks a written answer. The hook consumes it on the next prompt and skips reviewing that one message, so a quiz answer is not logged as your own writing. Consumed whether or not it fires, and ignored once older than 30 minutes. |
