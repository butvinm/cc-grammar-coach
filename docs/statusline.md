...
| `drill-active`                               | Present only while a quiz is running. The hook exits early when it exists, so answers are not logged as your own writing; a copy older than an hour is stale and gets removed on the next prompt. |
