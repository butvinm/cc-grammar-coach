...
| Route                    | Returns                                                                                                                                                                                                                                                                                                                                       |
| ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GET /` , `GET /duo.css` | the two app files from the server's own directory; no other path maps to the filesystem. The JS string literal `"__GRAMMAR_HOME__"` in `index.html` is replaced on the way out with the home this server serves, JSON-quoted - that is how the page can print a restart command that works in a shell where `GRAMMAR_HOME` was never exported |
...
| `GET /api/drills`        | newest-first summaries `{file, kind, date, topics: [{label, count}]}` over `drills/*.json`; legacy `drills/*.html` is ignored, and so is any file whose `topics` or `questions` is not a non-empty list - a card that cannot open is worse than no card                                                                                       |
| `GET /api/drills/<file>` | one drill JSON; only a `.json` basename resolves, anything else 404s. A file that is present but unreadable or not JSON answers 500, never 404: the page renders 404 as "Drill not found" and any other status as "Drill unreadable"                                                                                                          |
